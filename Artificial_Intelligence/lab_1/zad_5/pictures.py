...
def changeSublist(picture, index):
    ...
    for i in range(len(picture[index])):
        ...
        a = minNumberOfFlips(picture[index], getNums(picture)[index])
        b = minNumberOfFlips(secondObject(picture)[i], getNums(secondObject(picture))[i])
        flipsSum = a + b
        # print(i, a, b)
...
def isCorrect():
    for i in range(len(rowList)):
        if minNumberOfFlips(rowList[i], rowNums[i]) != 0:
            return False
    for i in range(len(colList)):
        if minNumberOfFlips(colList[i], colNums[i]) != 0:
            return False
    return True
